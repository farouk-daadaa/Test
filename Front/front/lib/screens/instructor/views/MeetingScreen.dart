...
class _MeetingScreenState extends State<MeetingScreen> implements HMSUpdateListener {
  ...
  void initState() {
    ...
    _peers = widget.peers.where((peer) => peer.name.isNotEmpty &&
        peer.name != "Unknown").toSet().toList();
  ...
  Future<void> _loadChatMessages() async {
    ...
    final String? messagesJson = prefs.getString(
        'chat_messages_${widget.meetingToken}');
    if (messagesJson != null) {
      try {
        final List<dynamic> messagesList = jsonDecode(messagesJson);
        setState(() {
          _messages.addAll(
              messagesList.map((msg) => ChatMessage.fromJson(msg)));
        });
  ...
  Future<void> _saveChatMessages() async {
    final prefs = await SharedPreferences.getInstance();
    final List<Map<String, dynamic>> messagesList = _messages.map((msg) =>
        msg.toJson()).toList();
    await prefs.setString(
        'chat_messages_${widget.meetingToken}', jsonEncode(messagesList));
  ...
  Future<void> _fetchLocalPeerAndSetup() async {
    try {
      ...
      if (_localPeer != null) {
        print('Local peer fetched: ${_localPeer!.name}, peerId: ${_localPeer!
            .peerId}, metadata: ${_localPeer!.metadata}, role: ${_localPeer!
            .role.name}');
        _isInstructor = _localPeer!.role.name.toLowerCase() == "instructor";
        if (!_isInstructor && _localPeer!.metadata != null &&
            _localPeer!.metadata!.isNotEmpty) {
          try {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          print(
              'Initial setup - Attempt $i: Current Video: $currentVideoState, Desired: $_isVideoOn, Current Audio: $currentAudioState, Desired: $_isAudioOn');
          ...
          if (localPeer != null) {
            bool videoState = localPeer.videoTrack != null &&
                !localPeer.videoTrack!.isMute;
            bool audioState = localPeer.audioTrack != null &&
        ...
        if (localPeer != null) {
          setState(() {
            ...
            _isAudioOn =
                localPeer.audioTrack != null && !localPeer.audioTrack!.isMute;
          });
          print(
              'Meeting: Final initial states set - Video: $_isVideoOn, Audio: $_isAudioOn');
        }
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to fetch local peer: $e'),
      ...
      if (localPeer != null && _localPeer != null) {
        setState(() {
          ...
          if (_isVideoOn && videoTrack != null) {
            ...
            if (!_videoTracks.contains(videoTrack)) _videoTracks.add(
                videoTrack);
          } else {
            _peerIdToTrackMap.remove(_localPeer!.peerId);
            _videoTracks.removeWhere((track) =>
            track == _peerIdToTrackMap[_localPeer!.peerId]);
          }
        });
      }
    } catch (e) {
  ...
  Future<void> _toggleAudio() async {
    ...
    if (!_isInstructor && _isMutedByInstructor && !_isAudioOn) {
      _showNotification(
          "You cannot unmute yourself while muted by the instructor.");
      return;
    }
    ...
    try {
      ...
      HMSLocalPeer? localPeer = await widget.hmsSDK.getLocalPeer();
      if (localPeer != null) {
        bool newAudioState = localPeer.audioTrack != null &&
            !localPeer.audioTrack!.isMute;
        setState(() {
  ...
  Future<void> _toggleMuteAll() async {
    try {
      ...
      List<HMSRole> nonInstructorRoles = roles.where((role) =>
      role.name.toLowerCase() != "instructor").toList();
      ...
      if (nonInstructorRoles.isNotEmpty) {
        ...
        _showNotification(_isAllMuted
            ? "All participants muted"
            : "All participants unmuted");
      } else {
        _showNotification("No non-instructor roles found to ${_isAllMuted
            ? 'unmute'
            : 'mute'}");
      }
    } catch (e) {
  ...
  void _exitToMySessions() {
    Navigator.pushNamedAndRemoveUntil(
        context, '/my_sessions', (Route<dynamic> route) => false);
  }
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  bool isMuted = peer.isLocal ? !_isAudioOn : (peer.audioTrack
                      ?.isMute ?? true);
                  bool isPeerInstructor = peer.role.name.toLowerCase() ==
                      "instructor";
                  return ListTile(
                    ...
                    title: Row(
                      children: [
                        ...
                        if (!isPeerInstructor && _raisedHands.contains(peer
                            .peerId)) ...[
  ...
  Future<void> _toggleRaiseHand() async {
    ...
    try {
      ...
      if (_localPeer!.metadata == null || _localPeer!.metadata!.isEmpty) {
        ...
        try {
          currentMetadata =
          jsonDecode(_localPeer!.metadata!) as Map<String, dynamic>;
        } catch (e) {
      ...
      if (isHandRaised) {
        ...
        print('Lowered hand for ${_localPeer!.name}, peerId: ${_localPeer!
            .peerId}');
      } else {
        currentMetadata['handRaised'] = true;
        currentMetadata["handRaisedAt"] = DateTime
            .now()
        ...
        print('Raised hand for ${_localPeer!.name}, peerId: ${_localPeer!
            .peerId}');
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to toggle raise hand: $e'),
              backgroundColor: Colors.red),
        );
      }
    }
  ...
  void onJoin({required HMSRoom room}) {
    setState(() {
      _peers = (room.peers ?? []).where((peer) => peer.name.isNotEmpty &&
          peer.name != "Unknown").toSet().toList();
      for (var peer in _peers) {
        if (peer.metadata != null && peer.metadata!.isNotEmpty) {
          try {
            ...
            if (metadata['handRaised'] == true ||
                metadata.containsKey('handRaisedAt')) {
      ...
      print('Joined room with peers: ${_peers.map((p) => "${p.name} (peerId: ${p
          .peerId})").toList()}');
    ...
    setState(() {
      if (update == HMSPeerUpdate.peerJoined) {
        if (!_peers.any((p) => p.peerId == peer.peerId)) {
          ...
          print('Peer joined: ${peer.name}, peerId: ${peer
              .peerId}, raised hand: ${peer.metadata != null &&
              peer.metadata!.isNotEmpty &&
              (jsonDecode(peer.metadata!)['handRaised'] == true ||
                  jsonDecode(peer.metadata!).containsKey('handRaisedAt'))}');
        }
      } else if (update == HMSPeerUpdate.peerLeft) {
        _peers.removeWhere((p) => p.peerId == peer.peerId);
        _peerIdToTrackMap.remove(peer.peerId);
        _videoTracks.removeWhere((track) =>
        _peerIdToTrackMap[peer.peerId] == track);
  ...
  @override
  void onTrackUpdate(
      {required HMSTrack track, required HMSTrackUpdate trackUpdate, required HMSPeer peer}) {
    setState(() {
      if (track.kind == HMSTrackKind.kHMSTrackKindVideo) {
        ...
        print('Audio track updated for peer ${peer.name}: isMute=${track
            .isMute}');
      }
    });
  }
  ...
  void onError({required HMSException error}) {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Meeting error: ${error.message}'),
            backgroundColor: Colors.red),
      );
    }
  }
  ...
  void onMessage({required HMSMessage message, VoidCallback? updateChatUI}) {
    print('Received message: type=${message.type}, message=${message
        .message}, sender=${message.sender?.name}, senderPeerId=${message.sender
        ?.peerId}');
    setState(() {
      if (message.type == "typing") {
        if (message.sender != null &&
            message.sender!.peerId != _localPeer?.peerId) {
        ...
        if (message.message == "unmuted_by_instructor" && !_isInstructor) {
          _isMutedByInstructor = false;
        }
      } else {
        ...
        if (!isDuplicate) {
          String? usernameToStore = (message.sender?.peerId ==
              _localPeer?.peerId) ? widget.username : null;
  ...
  void onUpdateSpeakers({required List<HMSSpeaker> updateSpeakers}) {
    print(
        'Speakers updated: ${updateSpeakers.map((s) => s.peer.name).toList()}');
  }
  ...
  @override
  void onAudioDeviceChanged({HMSAudioDevice? currentAudioDevice, List<
      HMSAudioDevice>? availableAudioDevice}) {
    print(
        'Audio device changed: Current device: $currentAudioDevice, Available devices: $availableAudioDevice');
  }
  ...
  @override
  void onPeerListUpdate({required List<HMSPeer> addedPeers, required List<
      HMSPeer> removedPeers}) {
    print('Peer list updated: added ${addedPeers.length}, removed ${removedPeers
        .length}');
    setState(() {
      for (var peer in addedPeers) {
        ...
        if (!_peers.any((p) => p.peerId == peer.peerId)) {
          ...
          _showNotification('${peer.name} joined the meeting');
          print('Peer added: ${peer.name}, peerId: ${peer
              .peerId}, raised hand: ${peer.metadata != null &&
              peer.metadata!.isNotEmpty &&
              (jsonDecode(peer.metadata!)['handRaised'] == true ||
                  jsonDecode(peer.metadata!).containsKey('handRaisedAt'))}');
  ...
  void onSessionStoreAvailable({HMSSessionStore? hmsSessionStore}) {
    print('Session store available: ${hmsSessionStore != null
        ? "Initialized"
        : "Not initialized"}');
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<HMSPeer> nonInstructorPeers = _peers.where((peer) =>
    peer.role.name.toLowerCase() != "instructor").toList();
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 8),
                        // Add spacing between the two main sections
                        GestureDetector(
                          onTap: _showParticipantsList,
                          child: Container(
                            constraints: const BoxConstraints(maxWidth: 120),
                            // Reduced maxWidth to prevent overflow
                            padding: const EdgeInsets.symmetric(
                  ...
                  Expanded(
                    child: _peers.isEmpty
                      ...
                      children: [
                            ...
                            child: Stack(
                              children: [
                                ClipRRect(
                                  borderRadius: BorderRadius.circular(12),
                                  child: HMSVideoView(
                                    track: _peerIdToTrackMap[instructorPeer
                                        .peerId]!,
                                    setMirror: instructorPeer.peerId ==
                                        _localPeer?.peerId,
                                ...
                                Positioned(
                                  ...
                                  child: Container(
                                    ...
                                    child: Icon(
                                      instructorPeer.audioTrack?.isMute ?? true
                                          ? Icons.mic_off
                                          : Icons.mic,
                                      color: instructorPeer.audioTrack
                                          ?.isMute ?? true ? Colors.red : Colors
                                          .green,
                                      size: 20,
                                    ),
                                  ),
                        ...
                        Expanded(
                          child: nonInstructorPeers.isEmpty &&
                              !isInstructorVideoOn
                              ? const Center(
                            child: Text(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final hasVideo = videoTrack != null &&
                                  !videoTrack.isMute;
                              bool isMuted = peer.isLocal ? !_isAudioOn : (peer
                                  .audioTrack?.isMute ?? true);
                                  ...
                                  children: [
                                    ...
                                    Positioned(
                                      ...
                                      child: Container(
                                        ...
                                        child: Icon(
                                          isMuted ? Icons.mic_off : Icons.mic,
                                          color: isMuted ? Colors.red : Colors
                                              .green,
                                          size: 20,
                                        ),
                                      ),
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        _buildToggleButton(
                          icon: _isVideoOn ? Icons.videocam : Icons
                              .videocam_off,
                        ...
                        _buildToggleButton(
                          icon: Icons.chat,
                          color: _unreadMessageCount > 0 ? Colors.blue : Colors
                              .white,
                          onTap: _openChat,
                          isLoading: false,
                        ...
                        if (!_isInstructor)
                          _buildToggleButton(
                            ...
                            color: _raisedHands.contains(_localPeer?.peerId)
                                ? Colors.yellow
                                : Colors.white,
                        ...
                        if (_isInstructor)
                          _buildToggleButton(
                            icon: _isAllMuted ? Icons.volume_off : Icons
                                .volume_up,
    ...
    return StatefulBuilder(
      builder: (BuildContext context, StateSetter setBottomSheetState) {
        ({required HMSMessage message}) {
          this.onMessage(
              message: message, updateChatUI: () => setBottomSheetState(() {}));
        };
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isMe = message.senderUsername != null &&
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        GestureDetector(
                          onTap: () =>
                              _sendMessage(() => setBottomSheetState(() {})),
                          child: Container(
                            padding: const EdgeInsets.all(12),
                            decoration: BoxDecoration(
                              gradient: const LinearGradient(
